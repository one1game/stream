/* =====================================================
   ...
   Цепочка (все настройки — в MASTER ниже):
     1. EQ      : 24 Гц HPF → −2 дБ @55 (саб) → +2 дБ @110 (плотность)
                  → −2 дБ @320 (муть) → −3.2 дБ @3.2к (широкая выемка резкости)
                  → +0.5 дБ @5.2к (разборчивость) → −0.4 дБ от 9.5к
                  → LP 14.5 кГц (мягкая лента)
...
const MASTER = {
  eq: {
    ...
    // Замер тракта: у струнных, ambient, dark и jazzhop полоса 2–6 кГц стояла
    // на 6–9 дБ выше остальных слоёв, а у ambient наклон спектра был вообще
    // положительный (+0.7 дБ/окт) — это и есть «верх, который бьёт по ушам».
    // Поэтому здесь не подъём, а широкая выемка в зоне резкости (2–5 кГц,
    // Q 0.6 = почти полторы октавы) и лёгкий срез верха; presence оставлен
    // только как узкая точка разборчивости выше выемки.
    harsh:  { f: 3200,  gain: -3.2, q: 0.65 },
    presence:{ f: 5200, gain:  0.5, q: 0.7 },
    air:    { f: 9500,  gain: -0.4 },
    // Граница ленты опущена с 16 к: выше 14.5 к у лоу-фая ничего не живёт,
    // а всё, что там оставалось, только шипело.
    tapeLP: { f: 14500, q: 0.707 },
  },
